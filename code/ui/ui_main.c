  ...
  while ( 1 ) {
    ...
    if (token[0] == '{') {
	  ...
	  if (tempStr && (!Q_stricmp(tempStr, "female"))) {
        uiInfo.characterList[uiInfo.characterCount].base = String_Alloc("kyonshi");
      } else if (tempStr && (!Q_stricmp(tempStr, "male"))) {
        uiInfo.characterList[uiInfo.characterCount].base = String_Alloc("sergei");
	  } else {
        uiInfo.characterList[uiInfo.characterCount].base = String_Alloc("%s",tempStr);
	  }
